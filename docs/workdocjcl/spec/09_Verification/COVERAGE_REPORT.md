...
## 1. Summary（规模快照）
- 代码仓库扫描文件数（忽略常见 build 目录）：约 `2455`（见 `workdocjcl/inventory/repo_stats.json`）
- 规格文档 Markdown 文件数（`workdocjcl/spec/**/*.md`，含自动生成胶囊/索引/资产镜像）：`2594`
  - 逐文件胶囊（`workdocjcl/spec/10_File_Specs/by_path/**`）：`2431`
  - Rust crate specs（`workdocjcl/spec/11_Rust_Crate_Specs/**`）：`51`
  - Node package specs（`workdocjcl/spec/12_Node_Package_Specs/**`）：`5`
  - Indexes（`workdocjcl/spec/13_Indexes/**`）：`3`
  - Prompt 资产镜像（`workdocjcl/spec/04_Business_Logic/PROMPTS/**`）：`13`
  - System skills 资产镜像（`workdocjcl/spec/05_Integrations/SKILLS_SYSTEM_ARTIFACTS/**`）：`4`
...
- npm wrapper：平台探测、binary 选择、PATH/ENV 注入、信号转发
  - 覆盖：`workdocjcl/spec/07_Infrastructure/PACKAGING.md`
  - 源：`codex-cli/bin/codex.js`
- Rust CLI 顶层：子命令与默认交互模式的分发
  - 覆盖：`workdocjcl/spec/03_API/CLI.md`
  - 源：`codex-rs/cli/src/main.rs`

### 2.2 配置与环境（必须）
- `CODEX_HOME` 行为
  - 覆盖：`workdocjcl/spec/01_Configuration/ENVIRONMENT.md`
  - 源：`codex-rs/utils/home-dir/src/lib.rs`
- config 覆盖（`-c key=value`）语义
  - 覆盖：`workdocjcl/spec/03_API/CLI.md`、`workdocjcl/spec/08_Testing/UNIT_SPECS.md`
  - 源：`codex-rs/common/src/config_override.rs`
- features registry（key/stage/default）
  - 覆盖：`workdocjcl/spec/01_Configuration/FEATURE_FLAGS.md`
  - 源：`codex-rs/core/src/features.rs`

### 2.3 持久化（必须）
- state.sqlite schema（threads/logs/thread_dynamic_tools）
  - 覆盖：`workdocjcl/spec/02_Data/*.md`
  - 源：`codex-rs/state/migrations/*.sql`

### 2.4 核心运行模型（必须）
- turn loop / tool execution / approvals / sandbox（契约级）
  - 覆盖：`workdocjcl/spec/00_Overview/ARCHITECTURE.md`、`workdocjcl/spec/04_Business_Logic/*.md`、`workdocjcl/spec/07_Infrastructure/SECURITY_SANDBOX.md`
  - 源：`codex-rs/core/src/codex.rs`、`codex-rs/protocol/` 等

### 2.5 Prompt / Skills 资产（复刻必需）
- 系统预置 prompts（verbatim + checksums）
  - 覆盖：`workdocjcl/spec/04_Business_Logic/SYSTEM_PRESET_PROMPTS.md`、`workdocjcl/spec/04_Business_Logic/PROMPTS/INDEX.md`
  - 源：`codex-rs/core/*.md`、`codex-rs/core/templates/*`（编译期 `include_str!`）
- embedded system skills（verbatim + checksums）
  - 覆盖：`workdocjcl/spec/05_Integrations/SKILLS_SYSTEM_ARTIFACTS.md`
  - 源：`codex-rs/core/src/skills/assets/samples/*`（编译期 `include_dir!`）

### 2.6 集成（建议）
- model providers registry + auth injection + retry/timeout
  - 覆盖：`workdocjcl/spec/05_Integrations/MODEL_PROVIDERS.md`
  - 源：`codex-rs/core/src/model_provider_info.rs`
- MCP config + transports + OAuth
  - 覆盖：`workdocjcl/spec/05_Integrations/MCP.md`
  - 源：`codex-rs/core/src/config/types.rs`

## 3. Existence Coverage（无遗漏清单覆盖）
以下“清单类材料”用于保证仓库元素不会被遗漏（至少知道它存在）：
- repo-only 文件清单（**无遗漏基线**，不含 `workdocjcl/`）：`workdocjcl/inventory/file_manifest_repo.txt`
- 全量文件清单（含 `workdocjcl/` 生成物）：`workdocjcl/inventory/file_manifest.txt`
- 全量文件清单（更宽口径，含更多生成物）：`workdocjcl/inventory/file_manifest_all.txt`
- Rust workspace members：`workdocjcl/inventory/rust_workspace.md`
- Node workspace packages：`workdocjcl/inventory/node_workspace.md`
- 源码直读 env vars：`workdocjcl/inventory/env_vars_detected.txt`
- Rust 测试清单：`workdocjcl/inventory/rust_test_manifest.txt`
...
- 映射锚点：`workdocjcl/spec/09_Verification/CODE_TO_SPEC_MAP.md`
- 清单覆盖：`workdocjcl/inventory/*`

另外：`workdocjcl/spec/09_Verification/COVERAGE_REPORT.generated.md` 是技能模板产物（非仓库权威报告、内容不完整），仅保留用于追溯“为什么不采用该脚本”的原因。
